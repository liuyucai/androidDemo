...
public class FlowLayout extends ViewGroup {
     ...
     * 每行排列元素后，剩余宽度
     */
    private int surplusWidthSpace = 99999;
    ...
    public void setAdapter(List<?> list, int res, ItemView mItemView) {
        ...
        removeAllViews();
        int layoutPadding = dipToPx(getContext(), 10);  //设置子元素的padding
...
//        int size = list.size();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int count = getChildCount();  //viewGroup的方法
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);  //viewGroup的方法
    ...
    @Override
    protected void onLayout(boolean changed, int l, int t, int r, int b) {  //changed:子viw是否改变， l:left,t:top,r:right,b:bottom
        if (changed) {
    ...
    class Line {
        ...
        public void LayoutView(int l, int t) {
            ...
            if(isAlignByCenter == AlienState.FLOW_LAYOUT_CENTER){
                if(surplusWidth<surplusWidthSpace){
                    surplusWidthSpace = surplusWidth;
                }
            }
                    ...
                    if (i == 0) {
                        switch (isAlignByCenter) {  //默认1
                            ...
                            case AlienState.RIGHT:
                                left += surplusWidth;
                                break;
                            case AlienState.FLOW_LAYOUT_CENTER:
                                left = surplusWidthSpace/2;
//                                left = 0;
                                break;
                            default:
//                                left = surplusWidthSpace/2;
